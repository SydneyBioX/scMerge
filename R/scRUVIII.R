...
scRUVIII <- function(Y = Y, M = M, ctl = ctl, fullalpha = NULL, k = k, return.info = TRUE, cell_type = NULL, batch = NULL, return_all_RUV = TRUE, 
    ...
    ruv3_initial <- fastRUVIII(Y = t(normY), ctl = ctl, k = k[1], M = M, fullalpha = fullalpha, return.info = return.info, fast_svd = fast_svd, rsvd_prop = rsvd_prop)
    
    ruv3_initial$k <- k
    ################### The computed result is ruv3res_list.  If we have only one ruvK value, then the result is ruv3res_list, with only one element, corresponding to
    ################### our initial run.
    ...
    if (length(k) == 1) {
        ...
        ## If we have more than one ruvK value, then we feed the result to the ruv::RUVIII function (there is no need for fast_svd, since we already have
        ## the fullalpha)
        for (i in 2:length(k)) {
...
calculateSil = function(x, fast_svd, cell_type, batch) {
    ...
    return(c(kBET::batch_sil(pca.data, as.numeric(as.factor(cell_type))), kBET::batch_sil(pca.data, as.numeric(as.factor(batch)), nPCs = 10)))
}
