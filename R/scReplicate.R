...
scReplicate <- function(sce_combine, batch = NULL, kmeansK = NULL, exprs = "logcounts", 
    hvg_exprs = "counts", marker = NULL, marker_list = NULL, replicate_prop = 1, cell_type = NULL, 
    cell_type_match = FALSE, cell_type_inc = NULL, dist = "cor", WV = NULL, WV_marker = NULL, 
    parallelParam = SerialParam(), return_all = FALSE, fast_svd, verbose = FALSE) {
    ...
    if (!is.null(cell_type) & is.null(cell_type_inc) & !cell_type_match) {
        ...
        replicate_vector <- supervisedReplicate(exprs_mat, cell_type, replicate_prop)
        mnc_res <- NULL
        ...
        hvg_cases_output <- hvg_cases(sce_combine = sce_combine, hvg_exprs = hvg_exprs, 
            batch = batch, batch_list = batch_list, marker = marker, marker_list = marker_list, 
            parallelParam = parallelParam, verbose = verbose)
        ...
        ## If there are no marker information what so ever, we find the HVG adaptively from
        ## the data
        ...
        if (!is.null(WV)) {
            ...
            replicate_vector <- wvReplicate(exprs_mat, WV, WV_marker, replicate_vector)
        }
...
############################ 
compute_cluster_res = function(exprs_mat, batch, marker, HVG_list, kmeansK, parallelParam, 
    fast_svd, cell_type = NULL, batch_list = NULL, case, verbose) {
    ...
    if (case == "case2") {
        ...
        ## Now that all markers/HVG are defined, we prepare ourselves for clustering.  For
        ## every unique cell_type, calculate the centroid.
        ...
        cluster_res = list(clustering_list = cellType_list_batch, clustering_distProp = clustering_distProp_list_batch)
    }
...
hvg_cases = function(sce_combine, hvg_exprs, batch, batch_list, marker, marker_list, 
    parallelParam = parallelParam, verbose) {
    ...
    if (is.null(marker) & is.null(marker_list)) {
        ...
        HVG_res <- findHVG(exprs_mat_HVG, batch, parallelParam = parallelParam, verbose = verbose)
        HVG <- HVG_res$HVG
...
findHVG <- function(exprs_mat_HVG, batch, intersection = 1, fdr = 0.01, minBiolDisp = 0.5, 
    ...
    HVG_list <- BiocParallel::bplapply(batch_list, function(x) {
        zeros <- base::rowMeans(exprs_mat_HVG[, batch == x] == 0)
...
identifyCluster <- function(exprs_mat, batch, marker = NULL, HVG_list, kmeansK, parallelParam, 
    ...
    # if(ncol(exprs_mat)>=5000){ rpca_q = 0 }else if(ncol(exprs_mat)>=2000){ rpca_q =1
    # }else{ rpca_q =2 }
    ...
    # } else { pca <- lapply(batch_list, function(this_batch_list) {
    # computePCA_byHVGMarker(this_batch_list = this_batch_list, batch = batch,
    # batch_oneType = batch_oneType, marker = marker, exprs_mat = exprs_mat, HVG_list =
    # HVG_list, fast_svd = fast_svd) }) }
    ...
    res <- BiocParallel::bplapply(seq_len(length(pca)), function(j) {
        ...
        if (!is.null(pca_current)) {
            ...
            if (!is.null(marker)) {
                clustering_res_pt_dist_tmp <- centroidDist(exprs_mat[marker, batch == 
                  batch_list[j]])
            } else {
    ...
    # clustering_res_pt_dist[[j]][names(clustering_res[[j]])] } else {
    # clustering_res[[j]] <- rep(1, batch_num[j]) names(clustering_res[[j]]) <-
    # colnames(exprs_mat[, batch == batch_list[j]]) if (!is.null(marker)) {
    # clustering_res_pt_dist[[j]] <- centroidDist(exprs_mat[marker, batch ==
    # batch_list[j]]) } else { clustering_res_pt_dist[[j]] <-
    # centroidDist(exprs_mat[HVG_list[[j]], batch == batch_list[j]]) } } } }
...
findMNC <- function(exprs_mat, clustering_list, dist = "euclidean", parallelParam) {
    ...
    if (length(batch_oneType) != 0) {
        ...
        if (length(batch_oneType) == batch_num) {
            ...
            ## if at least some batch contains more than 1 cell type Then take away the batches
            ## with only one cell type and then iterate through all combn
    ...
    ## If there are only two batches containing only two cell types, then finding MNN is
    ## trivial. Return NULL
    if (allones & batch_num == 2) {
        ...
        # } else { dist_mat_med <- lapply(seq_len(ncol(combine_pair)), function(k) {
        # compute_dist_mat_med(k = k, exprs_mat = exprs_mat, clustering_list =
        # clustering_list, combine_pair = combine_pair, dist = dist) }) }
        ...
        ## The neighbour_res is then which ever two pairs of batches that are closes to each
        ## other
        ...
        for (k in seq_len(ncol(combine_pair))) {
            ...
            mnc[[k]] <- mnc_tmp
            colnames(mnc[[k]]) <- c(paste("Batch", combine_pair[1, k], sep = ""), paste("Batch", 
                combine_pair[2, k], sep = ""))
        }
    }  ## End else
...
supervisedReplicate <- function(exprs_mat, cell_type, replicate_prop) {
    ...
    ## For each unique value in the cell_type vector, calculate the centroid distance of
    ## those unique cell_types
    clustering_distProp <- lapply(unique(cell_type), function(x) centroidDist(exprs_mat[, 
        cell_type == x]))
...
###################################################################################################### Function to create replicates based on known cell types
computePCA_byHVGMarker <- function(this_batch_list, batch, batch_oneType, marker, exprs_mat, 
    HVG_list, fast_svd) {
    ...
    if (this_batch_list %in% batch_oneType) {
        ...
        ## If there exist some user-supplied markers, then we will use them to perform PCA.
        ## Otherwise we use the HVGs computed in each batch
        if (!is.null(marker)) {
        ...
        ## If fast_svd option was enabled, then we will use irlba to speed up the
        ## calculations.
        if (fast_svd) {
            result <- irlba::prcomp_irlba(x = sub_exprs_mat, n = 10, scale. = TRUE, 
                maxit = 1000)$x
        } else {
