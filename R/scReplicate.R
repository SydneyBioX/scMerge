...
scReplicate <- function(sce, batch = NULL, kmeansK = NULL, exprs = "logcounts", hvg_exprs = "counts", marker = NULL, marker_list = NULL, replicate_prop = 1, 
    cell_type = NULL, cell_type_match = FALSE, cell_type_inc = NULL, dist = "cor", WV = NULL, WV_marker = NULL, parallelParam = SerialParam(), return_all = FALSE, 
    fast_svd) {
...
findHVG <- function(exprs_mat_HVG, batch, intersection = 1, fdr = 0.01, minBiolDisp = 0.5, parallelParam) {
    ...
    HVG_list <- BiocParallel::bplapply(batch_list, function(x) {
        ...
        M3Drop::BrenneckeGetVariableGenes(exprs_mat_HVG[express_gene, batch == x], suppress.plot = TRUE, fdr = 0.01, minBiolDisp = 0.5)
    ...
    # express_gene <- names(which(zeros <= 0.9)) M3Drop::BrenneckeGetVariableGenes(exprs_mat_HVG[express_gene, batch == x], suppress.plot = TRUE, fdr
    # = 0.01, minBiolDisp = 0.5) }) }
...
identifyCluster <- function(exprs_mat, batch, marker = NULL, HVG_list, kmeansK, parallelParam, fast_svd) {
    ...
    # } else { for (j in 1:length(pca)) { pca_current <- pca[[j]] if (!is.null(pca_current)) { kmeans_res <- stats::kmeans(pca_current[, 1:10],
    # centers = kmeansK[j], nstart = 1000) clustering_res[[j]] <- kmeans_res$cluster # exprs_current <- exprs_mat[HVG_list[[j]], batch ==
    ...
    # centroidDist(exprs_current[, kmeans_res$cluster == y, drop = FALSE]) }) clustering_res_pt_dist[[j]] <- unlist(clustering_res_pt_dist[[j]])
    # clustering_res_pt_dist[[j]] <- clustering_res_pt_dist[[j]][names(clustering_res[[j]])] } else { clustering_res[[j]] <- rep(1, batch_num[j])
    # names(clustering_res[[j]]) <- colnames(exprs_mat[, batch == batch_list[j]]) if (!is.null(marker)) { clustering_res_pt_dist[[j]] <-
    # centroidDist(exprs_mat[marker, batch == batch_list[j]]) } else { clustering_res_pt_dist[[j]] <- centroidDist(exprs_mat[HVG_list[[j]], batch ==
    # batch_list[j]]) } } } }
...
findMNC <- function(exprs_mat, clustering_list, dist = "euclidean", parallelParam) {
    ...
    if (length(batch_oneType) != 0) {
        ...
        if (length(batch_oneType) == batch_num) {
            ...
            ## if at least some batch contains more than 1 cell type Then take away the batches with only one cell type and then iterate through all combn
    ...
    if (allones & batch_num == 2) {
        ...
        # } else { dist_mat_med <- lapply(seq_len(ncol(combine_pair)), function(k) { compute_dist_mat_med(k = k, exprs_mat = exprs_mat, clustering_list =
        ...
        for (k in seq_len(ncol(combine_pair))) {
            ...
            # } else { dist_res <- lapply(seq_len(max(res1)), function(i) { compute_dist_res(i = i, res1 = res1, res2 = res2, exprs_mat = exprs_mat, dist =
            # dist, dist_res) }) }
