...
findHVG <- function(exprs_mat_HVG, batch, intersection = 1, fdr = 0.01, minBiolDisp = 0.5, 
    ...
    HVG_list <- BiocParallel::bplapply(batch_list, function(x) {
        zeros <- apply(exprs_mat_HVG[, batch == x], 1, function(x) sum(x == 
            0)/length(x))
        express_gene <- names(which(zeros <= 0.9))
        M3Drop::BrenneckeGetVariableGenes(exprs_mat_HVG[express_gene, batch == 
            x], suppress.plot = TRUE, fdr = 0.01, minBiolDisp = 0.5)
    }, BPPARAM = parallelParam)
    # } else { HVG_list <- lapply(batch_list, function(x) { zeros <-
    # apply(exprs_mat_HVG[, batch == x], 1, function(x) sum(x == 0)/length(x))
    # express_gene <- names(which(zeros <= 0.9))
    # M3Drop::BrenneckeGetVariableGenes(exprs_mat_HVG[express_gene, batch == x],
    # suppress.plot = TRUE, fdr = 0.01, minBiolDisp = 0.5) }) }
...
identifyCluster <- function(exprs_mat, batch, marker = NULL, HVG_list, kmeansK, 
    parallelParam, fast_svd) {
    ...
    # centers = kmeansK[j], nstart = 1000) clustering_res[[j]] <-
    # kmeans_res$cluster # exprs_current <- exprs_mat[HVG_list[[j]], batch ==
    # batch_list[j]] if (!is.null(marker)) { exprs_current <- exprs_mat[marker,
    # batch == batch_list[j]] } else { exprs_current <- exprs_mat[HVG_list[[j]],
    # batch == batch_list[j]] } clustering_res_pt_dist[[j]] <-
    # lapply(1:kmeansK[j], function(y) { centroidDist(exprs_current[,
    # kmeans_res$cluster == y, drop = FALSE]) }) clustering_res_pt_dist[[j]] <-
    # unlist(clustering_res_pt_dist[[j]]) clustering_res_pt_dist[[j]] <-
...
compute_dist_mat_med <- function(k, exprs_mat, clustering_list, combine_pair, 
    dist) {
    ## We go through every pairwise batches print(k) Extract the cell type
    ## information and the expression matrices
...
findMNC <- function(exprs_mat, clustering_list, dist = "euclidean", parallelParam) {
    ...
    if (length(batch_oneType) != 0) {
        ## And if all batch_oneType == num of batches, i.e. every batch only contains
        ## one cell type
        if (length(batch_oneType) == batch_num) {
            ...
            ## And then for those batches with only one cell type, we bind to the
            ## previous generated combn
    ...
    ## If there are only two batches containing only two cell types, then finding
    ## MNN is trivial. Return NULL
    if (allones & batch_num == 2) {
        ...
        ## The neighbour_res is then which ever two pairs of batches that are closes
        ## to each other
...
mncReplicate <- function(clustering_list, clustering_distProp, replicate_prop, 
    mnc_df) {
    batch_num <- length(clustering_list)
    if (!is.null(mnc_df)) {
        idx_noRep <- list()
            ...
            if (length(idx_noRep[[j]]) != 0) {
                for (k in seq_along(idx_noRep[[j]])) {
                  ...
                  replicate_vector[tmp_names[clustering_distProp[tmp_names] <= 
                    replicate_prop]] <- paste("Replicate", current_idx + k, 
                    sep = "_")
                }
        ...
        for (j in seq_along(clustering_list)) {
            for (k in 1:max(clustering_list[[j]])) {
                ...
                replicate_vector[tmp_names[clustering_distProp[tmp_names] <= 
                  replicate_prop]] <- paste("Replicate", current_idx + k, sep = "_")
            }
...
wvReplicate <- function(exprs_mat, WV, WV_marker, replicate_vector) {
    ...
    if (!is.null(WV_marker)) {
        ...
        # repMat_new <- ruv::replicate.matrix(as.factor(paste(replicate_vector, WV,
        # sep = '_')))
    }
...
###################################################################################################### Function to create replicates based on known cell types
computePCA_byHVGMarker <- function(this_batch_list, batch, batch_oneType, marker, 
    exprs_mat, HVG_list, fast_svd) {
    ...
    if (this_batch_list %in% batch_oneType) {
        ...
        ## If there exist some user-supplied markers, then we will use them to
        ## perform PCA.  Otherwise we use the HVGs computed in each batch
        if (!is.null(marker)) {
            sub_exprs_mat <- t(exprs_mat[marker, batch == this_batch_list])
        } else {
            sub_exprs_mat <- t(exprs_mat[HVG_list[[this_batch_list]], batch == 
                this_batch_list])
        }
