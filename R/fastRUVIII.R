...
fastRUVIII <- function(Y, M, ctl, k = NULL, eta = NULL, fast_svd = FALSE, rsvd_prop = 0.1, 
    ...
    ## m represent the number of samples/observations ncol(M) represent the
    ## number of replicates If the replicate matrix is such that we have more
    ## replicates than samples, then RUV3 is not appropriate, thus, we return the
    ...
    if (ncol(M) >= m | k == 0) {
        ...
        if (is.null(fullalpha)) 
            {
                ## The main RUVIII process Applies the residual operator of a matrix M to a
                ## matrix Y Y0 has the same dimensions as Y, i.e. m rows (observations) and n
                ## columns (genes).
    ...
    ## If the users want to get all the informations relating to the RUV, it can
    ## be done here.
    if (!return.info) {
        return(newY)
    } else {
