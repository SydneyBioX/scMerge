...
sce_combine = sce_cbind(sce_list = sce_split, batch_names = batch_names, cut_off_batch = 0, cut_off_overall = 0, method = NULL, exprs = c("counts", 
    "logcounts"), colData_names = c("cellTypes"))
# expect_identical(example_sce, sce_combine) expect_equal(example_sce@assays, sce_combine@assays) expect_identical(example_sce@int_metadata,
# sce_combine@int_metadata) expect_identical(example_sce@reducedDims, sce_combine@reducedDims)
