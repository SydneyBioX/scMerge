...
We recommend only use this option when the number of cells is large in your single cell data. The speed advantage we obtain for large signgle cell data is more dramatic than on a smaller dataset like the example mESC data. (For instance, in a single cell data with 21700 features and 5526 cells, we saw a 5-fold reduction in computational time when `fast_svd = TRUE` was invoked.)
